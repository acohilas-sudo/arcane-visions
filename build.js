...
// Read intrinsic pixel dimensions straight from the file header — no external
// tool, so it works identically on macOS (local builds) AND on Linux (Cloudflare
// Pages Git builds). This previously shelled out to macOS `sips`, which silently
// returned null in the Linux production build and stripped every width/height
// from the output. Pure header parsing for the formats the atelier ships
// (JPEG/PNG/GIF/WebP); unknown formats return null and attributes are omitted
// rather than faked. No new dependency — just fs + Buffer.
function readImageSize(buf) {
  ...
  // JPEG: walk the marker segments to the Start-Of-Frame, which carries H/W.
  if (buf[0] === 0xff && buf[1] === 0xd8) {
    ...
    while (off + 9 < buf.length) {
      ...
      // SOF0-SOF15 carry frame dimensions; skip DHT(C4)/JPG(C8)/DAC(CC) and others.
...
function imgDims(refPath) {
  ...
  if (fs.existsSync(abs)) {
    try {
      const d = readImageSize(fs.readFileSync(abs));
      if (d && Number.isFinite(d.width) && Number.isFinite(d.height) && d.width > 0 && d.height > 0) {
        result = { width: d.width, height: d.height };
      }
    } catch (e) { /* leave null */ }
  }
